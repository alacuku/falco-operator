...
// preserved.
//
// The write mechanism depends on the direction of the change:
//   - Pure additions go through server-side apply (client.Apply with ForceOwnership), so
//     the controller owns the finalizers it manages.
//   - Removals go through a MergeFrom patch that replaces the whole list. SSA cannot
//     remove elements of metadata.finalizers (a listType=set field) that are owned or
//     co-owned by other managers: applying the same value only adds the applier as a
//     co-owner, it never strips the previous owner — so a legacy finalizer left by an old
//     operator version would survive any apply. A merge patch replaces the array
//     wholesale, which is the only patch form that can evict another manager's entries.
//
