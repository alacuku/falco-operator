...
// These tests run against a real API server (envtest) to prove the server-side-apply
// contract of ReconcileInUseFinalizer — ownership of added finalizers, removals of
// foreign-owned entries, and no-op reconciles not bumping resourceVersion — none of
// which the fake client implements faithfully for metadata.finalizers.
...
// so the finalizer elements are owned by an update manager — exactly how finalizers
// left behind by old operator versions (or kubectl) look to the SSA applier.
