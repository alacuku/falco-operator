...
func withRulesfileSelector(key, value string) func(*artifactv1alpha1.Rulesfile) {
	return func(rf *artifactv1alpha1.Rulesfile) {
		rf.Spec.Selector = &metav1.LabelSelector{MatchLabels: map[string]string{key: value}}
	}
}
...
func TestReconcile_DeletesStaleNodeObject(t *testing.T) {
	rf := newTestRulesfile(withRulesfileSelector("env", "prod"))
...
func TestFetchAndCacheArtifactMeta_OCICacheHitIgnoresDigest(t *testing.T) {
	// Spec hash matches even though the stored digest is stale — cache is still a hit.
