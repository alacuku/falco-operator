...
//   - Creating one RulesfileNode per cluster node that matches the Rulesfile selector.
//   - Deleting RulesfileNode objects when a node no longer matches.
...
// RulesfileAggregatorReconciler manages RulesfileNode objects and aggregates their
...
// Reconcile reconciles a Rulesfile: ensures RulesfileNode objects exist for matching nodes,
...
func (r *RulesfileAggregatorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Add the in-use finalizer before doing any work. This prevents the parent Rulesfile
	// from being garbage-collected while we still have RulesfileNode objects to clean up.
	...
	if err := controllerhelper.ReconcileInUseFinalizer(
		...
		len(allMatchingNodes) > 0,
	); err != nil {
		return ctrl.Result{}, err
	}

	// List all existing RulesfileNode objects for this Rulesfile.
	...
	// Build a set of node names that should have a RulesfileNode.
	...
	if err := r.fetchAndCacheArtifactMeta(ctx, rulesfile); err != nil {
		...
		// advanced, so enforce-mode per-node operators stay deferred, and no RulesfileNodes are
	...
	// Pulls the OCI binary before creating RulesfileNode objects, so the blob cache is warm
	...
	// Ensure a RulesfileNode exists for each matching node.
	...
	controllerhelper.ComputeAggregateConditions(ctx, rulesfile, &rulesfile.Status.Conditions, existingNodes)
...
// handleDeletion deletes all RulesfileNode objects so each per-node artifact operator can clean up
