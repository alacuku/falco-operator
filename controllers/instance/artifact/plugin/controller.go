...
// See the rulesfile package for a full description of the pattern.
package plugin
...
// PluginAggregatorReconciler manages PluginNode objects and aggregates their
...
// Reconcile reconciles a Plugin: ensures PluginNode objects exist for matching nodes,
...
func (r *PluginAggregatorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.fetchAndCacheArtifactMeta(ctx, plugin); err != nil {
		...
		// advanced, so enforce-mode per-node operators stay deferred, and no PluginNodes are
	...
	// Pulls the OCI binary for every platform present in matching nodes before creating PluginNode
	...
	// Ensure a PluginNode exists for each matching node.
	...
	controllerhelper.ComputeAggregateConditions(ctx, plugin, &plugin.Status.Conditions, existingNodes)
...
// handleDeletion deletes all PluginNode objects so each per-node artifact operator can clean up and
