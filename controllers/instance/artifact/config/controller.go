...
// ConfigAggregatorReconciler manages ConfigNode objects and aggregates their
...
// Reconcile reconciles a Config: ensures ConfigNode objects exist for matching nodes,
...
func (r *ConfigAggregatorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Ensure a ConfigNode exists for each matching node.
...
// handleDeletion deletes all ConfigNode objects so each per-node artifact operator can clean up and
